...
*/
    ...
    sortedArr = (int**)malloc(size * sizeof(int*));  //Allocate memory for the pointers array
    checkAlloc(sortedArr);
    setIntPointersArray(arr, size, sortedArr);       //Set the pointers in pointer array to points to the cells in the array

    //Check whether the value of ascend_flag is 1 or 0
    if (ascend_flag == 1)
    {
        mergeSortPointersArray(sortedArr, 0, size - 1, ascend_flag);
    }
    else if (ascend_flag == 0)
    {
        mergeSortPointersArray(sortedArr, 0, size - 1, ascend_flag);
